...
class CompressLinks:
    ...
    def _compress_part(self, part: str) -> str:
        length_of_part = len(part)
        start = part[0]
        end = part[-1]
        compress_number= length_of_part - 2
        s = str(compress_number)
        return start+s+end
         
    
    def get_result(self, link: str) -> str:
        link_tokens = link.split("/")
        ans = []
        for link_token in link_tokens:
            if "." in link_token:
                minor_parts = link_token.split(".")
                parts = []
                s = ""
                for part in minor_parts:
                    compress_part = self._compress_part(part)
                    parts.append(compress_part)
                s = ".".join(parts)  
                  
                ans.append(s)    
            else:
                compress_part = self._compress_part(link_token)
                ans.append(compress_part)
        
        return "/".join(ans)
    
    def get_compressed_parts(self, parts: str) -> str:
        start_letter = parts[0][0]
        ...
        end_letter = parts[total_length -1][len(parts[total_length -1]) -1]
        s = str(total)   
        return start_letter+s+end_letter   
                       
        
    
    def get_only_limited_compressed_parts(self, s: str, n: int) -> str:
        compressed_string = self.get_result(s)
        
        compressed_string_token = compressed_string.split("/")
        final_string = []
        for string in compressed_string_token:
                ...
                if len(tokens) >= n:
                    ...
                    for token in tokens:
                        ...
                        if len(parts) < n-1:
                            ...
                            compressed_parts = self.get_compressed_parts(tokens[n-1:])
                            parts.append(compressed_parts)  
                            break
                    s = ".".join(parts)  
...
if __name__ == "__main__":
    ...
    print(links.get_only_limited_compressed_parts("section/how.toooo.write.and.java.program.ino.one.day", 6))          
